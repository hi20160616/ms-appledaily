...
func NewArticle() *Article {
	return &Article{
		WebsiteDomain: configs.Data.MS["appledaily"].Domain,
		WebsiteTitle:  configs.Data.MS["appledaily"].Title,
		WebsiteId:     fmt.Sprintf("%x", md5.Sum([]byte(configs.Data.MS["appledaily"].Domain))),
	}
}
...
func (a *Article) Get(id string) (*Article, error) {
	...
	return nil, fmt.Errorf("[%s] no article with id: %s",
		configs.Data.MS["appledaily"].Title, id)
}
